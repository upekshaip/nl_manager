          ...
          child: Column(
            ...
            children: [
              ...
              if (myCourse.isLoading)
                MyLoading(message: "🔍 Scanning NLearn Files..."),
              ...
              if (!myCourse.isLoading &&
                  myDownloader.needsToDownload.isNotEmpty)
                Expanded(
                    ...
                    children: [
                      Container(
                        ...
                        child: Column(children: [
                          if (!myDownloader.isProcessing &&
                              !myDownloader.isDownloading)
                            Column(
                          ...
                          if (myDownloader.isDownloading &&
                              !myDownloader.isProcessing)
                            Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(
                                      "Total File Count: ${myDownloader.needsToDownload.length}"),
                                  Text(
                                      "Downloaded File Count: ${myDownloader.downloaded.length}"),
                                  Text(
                                      "File name: ${myDownloader.downloadingFileName}"),
                                  Text(
                                      "Path: ${myDownloader.downloadingFilePath}"),
                                  const SizedBox(height: 8),
                                  Text(
                                      "File Size: ${MyHelper().formatBytes(myDownloader.downloadingFileSize)}"),
                                  ...
                                  Row(children: [
                                    Expanded(
                                      child: LinearProgressIndicator(
                                        value:
                                            myDownloader.currentProgress ?? 0,
                                  ...
                                  Row(children: [
                                    ...
                                    const SizedBox(width: 8),
                                    Text(
                                        "${(myDownloader.totalProgress * 100).toStringAsFixed(2)}%"),
                                  ]),
                                  // PrimaryBtn(
                                  //   onTap: () {
                      ...
                      Expanded(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return ListTile(
                              title: Text(
                                myDownloader.needsToDownload[index]
                                    ["file_name"],
                                style: TextStyle(color: Colors.white),
                              ),
                              subtitle: Text(
                                myDownloader.needsToDownload[index]["path"]
                                    .toString()
                                    .split('/')
                                    .first,
                              ...
                              trailing: MyHelper().getIcons(
                                  myDownloader.needsToDownload[index]["ext"],
                                  myDownloader.needsToDownload[index]["image"]),
                            );
                          },
                        ),
                      ),
