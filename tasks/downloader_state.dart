...
class DownloadStateProvider extends ChangeNotifier {
  ...
  Future getMissingFiles(
      SessionStateProvider mySession, CourseStateProvider myCourse) async {
    resetValues();
    isProcessing = true;
    ...
    List missingFiles =
        await MyHelper().onlyGetMissingFiles(myCourse.courseData);
  ...
  Future startDownload(SessionStateProvider mySession) async {
        ...
        if (streamedResponse.statusCode == 200) {
          ...
          streamedResponse.stream.listen(
            (List<int> chunk) {
              ...
              currentProgress = (downloadingFileDownloaded.toDouble() /
            ...
            onDone: () async {
              ...
              totalProgress = needsToDownload.isEmpty
                  ? 1
                  : (downloaded.length / needsToDownload.length);
          ...
          await MyHelper().showProgressNotification(
              title: "📥 Downloading NLearn Files...",
    ...
    await MyHelper().showNotification(
        ...
        body:
            "Your files have been downloaded successfully! Make sure to refresh again to see the latest updates.");
